...
export const OfferUsageSection = component$<OfferUsageSectionProps>(
  ({ uid, variant = "card" }) => {
    ...
    useVisibleTask$(({ track, cleanup }) => {
      ...
      const unsubscribeEntitlement = watchEntitlement(
        value,
        (nextEntitlement) => {
    ...
    const content = () => {
      if (!entitlement.value) {
        if (variant === "inline") {
          return (
            <div
              class="ui-offer-usage-inline ui-skeleton-stack-sm"
              aria-hidden="true"
            >
      ...
      const remaining =
        offerLimit == null ? null : Math.max(0, offerLimit - used);
      ...
      const paidPlan = billingPlans.find(
        (plan) => plan.offerLimit !== null && Boolean(plan.priceId),
      );
        ...
        return (
          <div class="ui-offer-usage-inline">
            <div class="ui-offer-usage-inline-copy">
              <p class="ui-offer-usage-inline-title">
                {t(i18n, "offersRemainingTitle", "Offers remaining")}
              </p>
            ...
            {action}
            {status.value ? (
              <p class="ui-status ui-status-error">{status.value}</p>
            ) : null}
          </div>
        );
      }
