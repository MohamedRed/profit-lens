...
const qwikInstallConfigPath = path.join(root, 'apps', 'qwik', 'src', 'lib', 'config', 'install-defines.ts');
const definesPath = process.env.DEFINES_FILE || path.join(root, 'tool', 'dev_runtime_defines.json');
const androidAppDownloadSuffix = (process.env.ANDROID_APP_DOWNLOAD_SUFFIX || '').trim();
...
const appendDownloadSuffix = (value, suffix) => {
  const trimmed = String(value || '').trim();
  if (!trimmed || !suffix) {
    return trimmed;
  }
  return `${trimmed}${trimmed.includes('?') ? '&' : '?'}v=${encodeURIComponent(suffix)}`;
};

const installBody = [
  '// Generated by tool/sync_web_runtime_config.sh. Do not edit directly.',
  'export const installDefines = {',
  `  androidAppDownloadUrl: ${JSON.stringify(appendDownloadSuffix(defines.ANDROID_APP_DOWNLOAD_URL ?? '', androidAppDownloadSuffix))},`,
  '} as const;',
  '',
].join('\n');
fs.writeFileSync(qwikInstallConfigPath, installBody, 'utf8');
NODE

echo "Synced Firebase runtime config for Qwik + admin, billing defines, and install defines."
